...
exports.default = (0, config_1.registerAs)('storage', () => ({
    ...
    limits: {
        image: {
            ...
            allowedMimeTypes: (process.env.ALLOWED_IMAGE_TYPES || 'image/jpeg,image/png,image/gif,image/webp').split(','),
        },
        video: {
            maxSize: (parseInt(process.env.MAX_VIDEO_SIZE_MB || '100', 10)) * 1024 * 1024,
            allowedMimeTypes: (process.env.ALLOWED_VIDEO_TYPES ||
                'video/mp4,video/quicktime,video/x-msvideo,video/webm').split(','),
        },
    },
    imageProcessing: {
        thumbnail: {
